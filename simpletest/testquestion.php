...
require_once($CFG->dirroot . '/question/type/turtipskupon/simpletest/helpers.php');

/**
 * Unit tests for the multiple choice, multiple response question definition class.
 *
 */
class qtype_turtipskupon_single_question_test extends UnitTestCase {

    public function test_get_expected_data() {
        $question = test_turtipskupon_question_maker::make_a_turtipskupon_single_question();
    ...
    public function test_grading_rounding_three_right() {
        question_bank::load_question_definition_classes('turtipskupon');
        $mc = new qtype_turtipskupon_multi_question();
        test_turtipskupon_question_maker::initialise_a_question($mc);
        $mc->name = 'Odd numbers';
        $mc->questiontext = 'Which are the odd numbers?';
        $mc->generalfeedback = '1, 3 and 5 are the odd numbers.';
        $mc->qtype = question_bank::get_qtype('turtipskupon');

        $mc->shuffleanswers = 0;
        $mc->qdifficulty = '0';

        test_turtipskupon_question_maker::set_standard_combined_feedback_fields($mc);
    ...
    public function test_summarise_response() {
        $mc = test_turtipskupon_question_maker::make_a_turtipskupon_single_question();
        $mc->shuffleanswers = false;
        $mc->start_attempt(new question_attempt_step(), 1);

        $summary = $mc->summarise_response(array('answer' => 0),
                test_turtipskupon_question_maker::get_a_qa($mc));
 ...
 *
 */
class qtype_turtipskupon_multi_question_test extends UnitTestCase {

    public function test_get_expected_data() {
        $question = test_turtipskupon_question_maker::make_a_turtipskupon_multi_question();
        $question->start_attempt(new question_attempt_step(), 1);
    ...
    public function test_summarise_response() {
        $mc = test_turtipskupon_question_maker::make_a_turtipskupon_multi_question();
        $mc->shuffleanswers = false;
        $mc->start_attempt(new question_attempt_step(), 1);
